...
from osbenchmark.builder import cluster_config, provisioner
...
class BareProvisionerTests(TestCase):
    ...
    def test_prepare_without_plugins(self, mock_rm, mock_ensure_dir, mock_decompress):
        ...
        cluster_config.ClusterConfig(
            names="unit-test-cluster-config",
            root_path=None,
            config_paths=[HOME_DIR + "/.benchmark/benchmarks/cluster_configs/default/my-cluster-config"],
        ...
        self.assertEqual(HOME_DIR + "/.benchmark/benchmarks/cluster_configs/default/my-cluster-config", source_root_path)
...
class OpenSearchInstallerTests(TestCase):
    ...
    def test_prepare_default_data_paths(self, mock_rm, mock_ensure_dir, mock_decompress):
        installer = provisioner.OpenSearchInstaller(cluster_config=cluster_config.ClusterConfig(names="defaults",
                                                                    root_path=None,
                                                                    config_paths="/tmp"),
                                                       java_home="/usr/local/javas/java8",
                                                       node_name="benchmark-node-0",
...
class DockerProvisionerTests(TestCase):
    ...
    def test_provisioning_with_defaults(self, uuid4):
        ...
        c = cluster_config.ClusterConfig("unit-test-cluster-config", None, "/tmp", variables={
