...
class ClusterConfigLoaderTests(TestCase):
    ...
    def setUp(self):
        ...
        self.loader = cluster_config.ClusterConfigLoader(self.cluster_config_dir)
    ...
    def test_load_known_cluster_config(self):
        test_cluster_config = cluster_config.load_cluster_config(
            self.cluster_config_dir, ["default"],
            cluster_config_params={"data_paths": ["/mnt/disk0", "/mnt/disk1"]})
        self.assertEqual("default", test_cluster_config.name)
        self.assertEqual(
            [os.path.join(current_dir, "data", "cluster_configs", "v1", "vanilla", "templates")],
            test_cluster_config.config_paths)
        self.assertIsNone(test_cluster_config.root_path)
        self.assertDictEqual({
            "heap_size": "1g",
            "clean_command": "./gradlew clean",
            "data_paths": ["/mnt/disk0", "/mnt/disk1"]
        }, test_cluster_config.variables)
        self.assertIsNone(test_cluster_config.root_path)

    def test_load_cluster_config_with_mixin_single_config_base(self):
        test_cluster_config = cluster_config.load_cluster_config(self.cluster_config_dir, ["32gheap", "ea"])
        self.assertEqual("32gheap+ea", test_cluster_config.name)
        self.assertEqual(
            [os.path.join(current_dir, "data", "cluster_configs", "v1", "vanilla", "templates")],
            test_cluster_config.config_paths)
        self.assertIsNone(test_cluster_config.root_path)
    ...
    def test_load_cluster_config_with_mixin_multiple_config_bases(self):
        test_cluster_config = cluster_config.load_cluster_config(self.cluster_config_dir, ["32gheap", "ea", "verbose"])
        self.assertEqual("32gheap+ea+verbose", test_cluster_config.name)
        self.assertEqual([
            os.path.join(current_dir, "data", "cluster_configs", "v1", "vanilla", "templates"),
    ...
    def test_load_cluster_config_with_install_hook(self):
        test_cluster_config = cluster_config.load_cluster_config(
            ...
            ["default", "with_hook"],
            cluster_config_params={"data_paths": ["/mnt/disk0", "/mnt/disk1"]})
        self.assertEqual("default+with_hook", test_cluster_config.name)
        self.assertEqual([
            os.path.join(current_dir, "data", "cluster_configs", "v1", "vanilla", "templates"),
    ...
    def test_load_cluster_config_with_multiple_bases_referring_same_install_hook(self):
        ...
        self.assertEqual("with_hook+another_with_hook", test_cluster_config.name)
        self.assertEqual([
            os.path.join(current_dir, "data", "cluster_configs", "v1", "vanilla", "templates"),
            os.path.join(current_dir, "data", "cluster_configs", "v1", "with_hook", "templates"),
            os.path.join(current_dir, "data", "cluster_configs", "v1", "verbose_logging", "templates")
