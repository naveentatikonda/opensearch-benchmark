...
def list_cluster_configs(cfg):
    loader = ClusterConfigLoader(cluster_config_path(cfg))
    cluster_configs = []
...
def load_cluster_config(repo, name, cluster_config_params=None):
    ...
    for n in name:
        descriptor = ClusterConfigLoader(repo).load_cluster_config(n, cluster_config_params)
        ...
        for p in descriptor.root_paths:
            ...
            if BootstrapHookHandler(Component(root_path=p, entry_point=ClusterConfig.entry_point)).can_load():
                if not root_path:
                    root_path = p
    ...
    return ClusterConfig(name, root_path, all_config_paths, variables=variables)
...
class ClusterConfigLoader:
    ...
    def load_cluster_config(self, name, cluster_config_params=None):
        ...
        cluster_config_type = self._value(config, ["meta", "type"], default="cluster-config")
        ...
        if len(config_paths) == 0:
            self.logger.info("ClusterConfig [%s] does not define any config paths. Assuming that it is used as a mixin.", name)
        ...
        return ClusterConfigDescriptor(
...
class ClusterConfig:
    ...
    def mandatory_var(self, name):
        try:
            ...
            raise exceptions.SystemSetupError("ClusterConfig \"{}\" requires config key \"{}\"".format(self.name, name))
...
class BootstrapHookHandler:
    ...
    def __init__(self, component, loader_class=modules.ComponentLoader):
        ...
        In practice, this is a PluginDescriptor or a ClusterConfig instance.
